...
fn merkle_tree(filename: String) -> String {
    ...
    let vectors_of_words: Vec<String> = content.lines().into_iter().map(|s| s.to_string()).collect();

    let n :u32 = vectors_of_words[0].parse().unwrap();

    let mut data:Vec<String> = vectors_of_words[1..vectors_of_words.len()].to_vec();
